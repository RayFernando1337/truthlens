...
export function InputHeader({ isRecording, busy, demoOpen, onToggleDemo, onLoadFixture, onStopRecording, onStartRecording }: {
  ...
  return (
    <div className="flex items-center justify-between border-b border-border px-4 py-3">
      <span className="text-[11px] font-semibold uppercase tracking-widest text-text-secondary">Input</span>
      <div className="flex items-center gap-3">
        {!isRecording && (
          <div className="relative">
            <button type="button" onClick={onToggleDemo} disabled={busy} data-testid="demo-toggle"
              className="text-[11px] uppercase tracking-wider text-text-secondary transition-colors hover:text-foreground disabled:opacity-30">
              Demo {demoOpen ? "\u25B4" : "\u25BE"}
            </button>
        ...
        <button type="button" onClick={isRecording ? onStopRecording : onStartRecording}
          ...
          className={`flex items-center gap-1.5 text-[11px] uppercase tracking-wider transition-colors disabled:opacity-30 ${
            isRecording ? "text-accent" : "text-text-secondary hover:text-foreground"
          }`}
          title={isRecording ? "Stop recording" : "Start voice input"}>
          <span className={`inline-block h-2 w-2 rounded-full ${isRecording ? "animate-pulse bg-accent" : "bg-foreground/30"}`} />
    ...
    <div className="flex h-full flex-col">
      ...
      {showJumpButton && (
        <button type="button" onClick={onScrollToBottom}
          className="absolute bottom-2 left-1/2 z-10 -translate-x-1/2 border border-border bg-bg px-3 py-1.5 text-[10px] font-semibold uppercase tracking-widest text-foreground shadow-lg transition-colors hover:border-foreground">
          Scroll to live
        </button>
      )}
      <div ref={scrollRef} onScroll={onScroll} className="min-h-0 flex-1 overflow-y-auto">
        {voiceTranscript.length === 0 ? (
          <p className="text-sm text-text-secondary">Speak clearly. Transcription will appear here as audio is processed...</p>
        ) : (
          <div className="space-y-2">
            {voiceTranscript.map((chunk, i) => {
              const borderClass = voiceChunkSeverities
                ...
                : "";
              return (
                <p key={i} id={`tl-transcript-chunk-${i}`} className={`text-sm leading-relaxed text-foreground ${borderClass}`}>
                  <span className="mr-2 text-[10px] tabular-nums text-text-secondary">[{i + 1}]</span>
                  {chunk}
                </p>
              );
            })}
    ...
    <div className="flex h-full flex-col gap-3">
      ...
      {!showVoiceProtocol && (
        <textarea value={text} onChange={(e) => onTextChange(e.target.value)} disabled={busy}
          ...
          className="h-full w-full resize-none bg-transparent text-sm leading-relaxed text-foreground placeholder:text-text-secondary focus:outline-none disabled:opacity-50" />
      )}
    </div>
  );
}
  ...
  return (
    <div className="border-t border-border px-4 py-3">
      <div className="flex items-center gap-3">
        ...
        <span className="text-[10px] tabular-nums text-text-secondary">{label}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="border-t border-border px-4 py-3">
      <button type="button" onClick={onAction} disabled={!text.trim() || busy} data-testid="analyze-button"
        className="w-full border border-border py-2 text-[11px] font-semibold uppercase tracking-widest text-foreground transition-colors hover:border-foreground hover:bg-foreground hover:text-bg disabled:opacity-30 disabled:hover:border-border disabled:hover:bg-transparent disabled:hover:text-foreground">
