...
export default function TrustChart({
  ...
  const overlayD =
    overlay && overlay.length > 1 ? buildPath(overlay).d : null;

  return (
    <div className="bg-[#0f0f0f] px-4 py-3">
      <div className="flex items-baseline justify-between">
        <span className="text-[9px] font-semibold uppercase tracking-widest text-[#555]">
          Trust
        </span>
        <span
      ...
      >
        ...
        <line
          ...
          stroke="#222" strokeWidth="0.5" strokeDasharray="4 4"
        />
        <path d={main.fill} fill="url(#tp-fill)" />
        <path d={main.d} fill="none" stroke={color} strokeWidth="1.5" />
        {overlayD && (
          <path
            d={overlayD} fill="none" stroke="#e5e5e5"
            strokeWidth="0.75" strokeDasharray="3 2" opacity="0.35"
          />
        )}
        {main.pts.map((p, i) => (
          <circle
            key={i} cx={p.x} cy={p.y} r="1.5"
            fill={trustColor(scores[i])}
          />
        ))}
      </svg>
    </div>
